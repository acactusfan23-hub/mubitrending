...
def improved_source():
    ...
    for page_num in range(1,11):
        ...
        print(f'MUBI via Jina (cache-busted dual format): page {page_num}: {target}')
    ...
    if len(all_items) < MAX_ITEMS:
        print(f'  WARNING: only {len(all_items)} source films captured; existing safety checks will prevent an unsafe MDBList update.')
    if not all_items: raise RuntimeError('Cache-busted MUBI extraction returned no film links.')
    print('MUBI source ranking captured (cache-busted dual-format):')
