...
S = requests.Session()
S.headers.update({'User-Agent': 'Mozilla/5.0 Chrome/131 Safari/537.36', 'Accept-Language': 'en-GB,en;q=0.9'})
...
def get_mubi_films():
    # MUBI's collection is client-rendered, so requests/BeautifulSoup alone sees no film cards.
    with sync_playwright() as p:
        ...
        last = 0; stable = 0
        for _ in range(15):
            count = page.locator('a[href*="/en/gb/films/"]').count()
            if count == last: stable += 1
            else: stable = 0; last = count
            if stable >= 2 and count >= 10: break
            page.mouse.wheel(0, 5000); page.wait_for_timeout(1200)
        links = page.locator('a[href*="/en/gb/films/"]').evaluate_all("""
          els => els.map(a => ({href:a.href, title:a.getAttribute('aria-label') || a.getAttribute('title') || a.textContent.trim()}))
        """)
        browser.close()
    films=[]; seen=set()
    for x in links:
        m=re.search(r'/en/gb/films/([^/?#]+)', x.get('href',''))
        if not m: continue
        slug=m.group(1)
        if slug in seen: continue
        seen.add(slug)
        title=(x.get('title') or '').strip() or slug.replace('-',' ')
    ...
    print(f'MUBI Trending: found {len(films)} ranked film links.')
    return films
...
def mdb_get_list():
    ...
    r=S.get(f'{MDB_API}/list',params={**mdb_params(),'id':MDBLIST_LIST_ID},timeout=30); r.raise_for_status(); return r.json()
...
def mdb_remove(item):
    ...
    if item.get('imdb_id'): payload['imdb_id']=item['imdb_id']
    elif item.get('tmdb_id'): payload['tmdb_id']=item['tmdb_id']
    else: return
