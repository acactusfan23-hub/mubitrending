...
JINA_BASE = os.getenv('JINA_BASE', 'https://r.jina.ai/http://')
...
def jina_url(target):
    # Jina Reader accepts the target URL after the reader hostname. Percent-encode only the target query.
    p=urllib.parse.urlsplit(target)
    base=f'{p.scheme}://{p.netloc}{p.path}'
    if p.query: base += '?' + p.query
    return JINA_BASE + base

def parse_mubi_markdown(text):
    items=[]; seen=set()
    # Markdown links generated by Jina. Also accept raw MUBI film URLs embedded in HTML/text.
    patterns=[
        re.compile(r'\[[^\]]+\]\((https?://mubi\.com/[^)]*?/films/[^)]+)\)'),
        re.compile(r'https?://mubi\.com/(?:en/)?(?:[a-z]{2}/)?films/[A-Za-z0-9%_-]+')
    ]
    urls=[]
    for pat in patterns:
        urls.extend(pat.findall(text or ''))
    for raw in urls:
        ...
        # Try to recover the markdown link's display title.
        title=slug.replace('-',' ')
        nearby=re.search(r'\[([^\]]+)\]\('+re.escape(raw.split('?')[0])+r'[^)]*\)',text or '')
        if nearby: title=html.unescape(nearby.group(1)).strip()
        seen.add(slug); items.append({'rank':len(items)+1,'title':title,'slug':slug,'url':f'https://mubi.com/en/gb/films/{slug}'})
...
def scrape_mubi_via_jina():
    ...
    for page_num in range(1,11):
        ...
        r=S.get(jina_url(target),timeout=90)
        r.raise_for_status()
        batch=parse_mubi_markdown(r.text)
        before=len(items)
        for x in batch:
...
def main():
    ...
    with open('data/latest.json','w',encoding='utf-8') as f: json.dump({'updated_at':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'source':'MUBI webpage via Jina Reader','items':matched,'unmatched':unresolved,'mdblist_list_id':MDBLIST_LIST_ID},f,ensure_ascii=False,indent=2)
    print(f'Updated MDBList static list {MDBLIST_LIST_ID} with {len(matched)} films from MUBI Trending; {len(unresolved)} unresolved.')
