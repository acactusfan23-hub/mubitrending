...
def jina_url(target):
    # Jina Reader syntax is simply https://r.jina.ai/<target-url>.
    # Keep the target query string intact so ?page=N reaches MUBI.
    return JINA_BASE.rstrip('/') + '/' + target

def parse_mubi_markdown(text):
    items=[]; seen=set()
    # Jina normally returns Markdown links. Also support raw URLs and HTML anchors if its format changes.
    ...
    for title_or_url, raw in pairs:
        ...
        seen.add(slug)
        items.append({'rank':len(items)+1,'title':title,'slug':slug,'url':f'https://mubi.com/en/gb/films/{slug}'})
    ...
    with sync_playwright() as p:
        ...
        for page_num in range(1,11):
            ...
            for x in links:
                ...
                slug=m.group(1)
                if slug in seen: continue
                title=(x.get('aria') or x.get('title') or x.get('text') or x.get('alt') or '').strip() or slug.replace('-',' ')
...
def scrape_mubi_web():
    ...
    if not items:
        raise RuntimeError('Could not obtain MUBI UK Trending film links. Direct GitHub access may be blocked and the external reader did not return film links. Refusing to modify MDBList.')
